...
class PluginDynamicdashDashboard {
    ...
    static function getCards(array $criteria = []): array {
        global $DB;
        ...
        $cache_key = self::buildCacheKey($criteria);
    ...
    private static function safeCall(string $method, string $where): ?array {
        try {
            return self::$method($where);
        } catch (\Throwable $e) {
            Toolbox::logError("DynamicDash: Erro no card $method: " . $e->getMessage());
            return null;
        }
    }

    private static function buildCacheKey(array $criteria): string {
        return md5(
        ...
        if (!empty($criteria)) {
            ...
            foreach ($grouped as $crit) {
                ...
                switch ((int)$field) {
                    ...
                    case 7:
                        if (is_numeric($value)) {
                            $conditions[] = "glpi_tickets.itilcategories_id = " . (int)$value;
                        }
                        break;
                    case 8:
                        if (is_numeric($value)) {
                            $conditions[] = "glpi_tickets.entities_id = " . (int)$value;
                        }
                        break;
                    case 4:
                        if (is_numeric($value)) {
                    ...
                    case 71:
                        if (is_numeric($value)) {
                            $conditions[] = "glpi_tickets.id IN (SELECT tickets_id FROM glpi_groups_tickets WHERE type = 2 AND groups_id = " . (int)$value . ")";
                        }
                        break;
                    case 65:
                        if (is_numeric($value)) {
                            $conditions[] = "glpi_tickets.id IN (SELECT tickets_id FROM glpi_groups_tickets WHERE type = 1 AND groups_id = " . (int)$value . ")";
                        }
                        break;
                    case 10:
                        if (is_numeric($value)) {
                    ...
                    case 15:
                        if ($searchtype === 'morethan') {
                            ...
                            $conditions[] = "glpi_tickets.date LIKE '%$safe_value%'";
                        }
                        break;
                    case 16:
                        if ($searchtype === 'morethan') {
    ...
    private static function groupCriteria(array $criteria): array {
        ...
        foreach ($criteria as $key => $value) {
            if (preg_match('/criteria\[(\d+)\]\[(\w+)\]/', $key, $matches)) {
                $grouped[$matches[1]][$matches[2]] = $value;
